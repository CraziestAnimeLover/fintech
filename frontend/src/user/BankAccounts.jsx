...
const BankAccounts = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isVerified) {
      return alert("Please verify bank account first ⚠️");
    }

    try {
      if (editingBankId) {
        await userAPI.updateBankAccount(editingBankId, {
          ...form,
          isVerified,
        });
  ...
  {banks.map((bank) => (
    <tr key={bank._id}>
      ...
      <td className="border p-2 flex gap-2">
        ...
        {!bank.isDefault && (
          <CheckCircle onClick={() => handleSetDefault(bank._id)} />
        )}
      </td>
        ...
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center">
          <div className="bg-white p-6 rounded w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="grid gap-3">
              ...
              {loadingIFSC && (
                <p className="text-blue-500 text-sm">Fetching bank...</p>
              )}
